...
const PLANS = [
  { key: "STARTER",  label: "Starter",  price: 270_000, maxStudents: 200,  maxBranches: 1, maxStaff: 3,  text: "text-neutral-300", ring: "border-neutral-700" },
  { key: "BUSINESS", label: "Business", price: 570_000, maxStudents: 500,  maxBranches: 3, maxStaff: 5,  text: "text-blue-300",    ring: "border-blue-900/40" },
  { key: "PREMIUM",  label: "Premium",  price: 870_000, maxStudents: 1000, maxBranches: 8, maxStaff: 12, text: "text-purple-300",  ring: "border-purple-900/40" },
];
...
export default function SettingsPage() {
  ...
  const platformStats = totals ? [
    { label: "O'quv markazlar", value: totals.orgs,     icon: Building2,     color: "text-blue-400" },
    { label: "O'quvchilar",     value: totals.students, icon: GraduationCap, color: "text-green-400" },
    { label: "O'qituvchilar",   value: totals.teachers, icon: Users,         color: "text-purple-400" },
  ] : [];

  const inputCls = "w-full px-3 py-2 bg-neutral-800 border border-neutral-700 rounded-lg text-[13px] text-white placeholder:text-neutral-600 outline-none focus:border-neutral-600";

  return (
    <div className="max-w-4xl mx-auto space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-xl font-black text-white">Sozlamalar</h1>
        <p className="text-sm text-neutral-500 mt-0.5">Hisob va platforma sozlamalari</p>
      </div>

      {/* Profil */}
      <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-5">
        <h2 className="text-[13px] font-bold text-white mb-4">Mening hisobim</h2>
        <div className="flex items-center gap-4">
          <div className="w-14 h-14 rounded-2xl bg-blue-500/20 border border-blue-500/30 flex items-center justify-center shrink-0">
            <span className="text-[20px] font-black text-blue-400">
              {user?.name?.[0]?.toUpperCase() ?? "A"}
            </span>
          </div>
          <div className="min-w-0">
            <p className="text-[15px] font-bold text-white truncate">{user?.name ?? "Admin"}</p>
            <p className="text-[12px] text-neutral-500">{user?.phone ?? "—"}</p>
            <span className="inline-flex items-center gap-1 mt-1.5 text-[10px] font-bold px-2 py-0.5 rounded-full bg-blue-900/50 text-blue-300">
      ...
      <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-5">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="text-[13px] font-bold text-white">Parolni o'zgartirish</h2>
        </div>
        <form onSubmit={submitPassword} className="space-y-3 max-w-sm">
          ...
          {msg && (
            <div className={cn(
              ...
              msg.type === "ok" ? "bg-emerald-900/20 text-emerald-300" : "bg-red-900/20 text-red-300",
            )}>
      ...
      {/* Platforma holati */}
      <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-5">
        <h2 className="text-[13px] font-bold text-white mb-4">Platforma holati</h2>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {platformStats.map((s) => {
            const Icon = s.icon;
            return (
              <div key={s.label} className="bg-neutral-800/50 border border-neutral-800 rounded-xl p-3">
                <Icon className={cn("w-4 h-4 mb-2", s.color)} />
                <p className="text-[18px] font-black text-white leading-none">{Number(s.value).toLocaleString()}</p>
          ...
          <div className="bg-neutral-800/50 border border-neutral-800 rounded-xl p-3">
            <Wallet className="w-4 h-4 mb-2 text-emerald-400" />
            <p className="text-[15px] font-black text-emerald-400 leading-none">{fmtMoney(totals?.revenue ?? 0)}</p>
      ...
      <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-5">
        <h2 className="text-[13px] font-bold text-white mb-1">Tarif rejalari</h2>
        <p className="text-[11px] text-neutral-500 mb-4">Markazlar sotib oladigan tariflar (ma'lumot uchun)</p>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {PLANS.map((p) => (
            <div key={p.key} className={cn("bg-neutral-800/40 border rounded-xl p-4", p.ring)}>
              <p className={cn("text-[13px] font-bold", p.text)}>{p.label}</p>
              <p className="text-[18px] font-black text-white mt-1">{fmtMoney(p.price)}<span className="text-[11px] font-medium text-neutral-500">/oy</span></p>
