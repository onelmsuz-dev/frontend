...
export default function SubscriptionsPage() {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <div>
        <h1 className="text-xl font-black text-neutral-900 dark:text-white">Obunalar va to'lovlar</h1>
      ...
      <div>
        <div className="flex items-center gap-2 mb-3">
          ...
          <h2 className="text-[15px] font-bold text-neutral-900 dark:text-white">Kutilayotgan to'lovlar</h2>
          {pending.length > 0 && (
            <span className="text-[11px] font-bold px-2 py-0.5 rounded-full bg-amber-50 dark:bg-amber-900/40 text-amber-700 dark:text-amber-300">{pending.length}</span>
          )}
        </div>
        ...
        {reqLoading ? (
          ...
          <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-2xl py-10 text-center">
            <CheckCircle className="w-7 h-7 mx-auto mb-2 text-neutral-300 dark:text-neutral-700" />
            <p className="text-[13px] text-neutral-400 dark:text-neutral-600">Kutilayotgan to'lov so'rovi yo'q</p>
          </div>
        ) : (
          <div className="space-y-2">
            {pending.map(r => (
              <div key={r.id} className="bg-white dark:bg-neutral-900 border border-amber-200 dark:border-amber-900/30 rounded-2xl p-4 flex items-center justify-between gap-4 flex-wrap">
                <div className="flex items-center gap-3 min-w-0">
                  <div className="w-10 h-10 rounded-xl bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-900/40 flex items-center justify-center shrink-0">
                    <Building2 className="w-4 h-4 text-amber-600 dark:text-amber-400" />
                  </div>
                  <div className="min-w-0">
                    <p className="text-[14px] font-semibold text-neutral-900 dark:text-white truncate">{r.organization?.name ?? "—"}</p>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <button onClick={() => review(r.id, "REJECT")} disabled={busy === r.id}
                    className="flex items-center gap-1.5 h-9 px-3 rounded-xl bg-neutral-100 dark:bg-neutral-800 bg-red-50 dark:bg-red-900/40 text-neutral-300 dark:text-neutral-700 dark:text-neutral-300 hover:text-red-400 text-[13px] font-semibold transition-colors">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        {isLoading ? (
          Array.from({ length: 3 }).map((_, i) => (
            <div key={i} className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-2xl p-4"><Skeleton className="h-7 w-32" /></div>
          ))
        ) : (
          <>
            <div className="bg-white dark:bg-neutral-900 border border-emerald-200 dark:border-emerald-900/30 rounded-2xl p-4">
              <div className="flex items-center gap-2 mb-2">
                <TrendingUp className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" />
                <p className="text-[11px] font-semibold text-neutral-500 uppercase tracking-wide">Faol MRR</p>
              </div>
              <p className="text-[20px] font-black text-emerald-600 dark:text-emerald-400">{fmtMoney(activeMrr)}</p>
            </div>
            <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-2xl p-4">
              ...
              <p className="text-[20px] font-black text-blue-600 dark:text-blue-400">{fmtMoney(totalMrr)}</p>
            </div>
            <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-2xl p-4">
              <div className="flex items-center gap-2 mb-2">
              ...
              <p className="text-[20px] font-black text-neutral-900 dark:text-white">{activeCount} <span className="text-[12px] font-medium text-neutral-400 dark:text-neutral-600">/ {orgs.length}</span></p>
            </div>
          </>
        )}
      </div>
