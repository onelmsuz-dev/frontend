...
const PLAN_CFG: Record<string, { label: string; cls: string }> = {
  STARTER:    { label: "Starter",  cls: "bg-neutral-200 dark:bg-neutral-700 text-neutral-300 dark:text-neutral-700 dark:text-neutral-300" },
  BUSINESS:   { label: "Business", cls: "bg-blue-50 dark:bg-blue-900/60 text-blue-700 dark:text-blue-300 border border-blue-800/40" },
  PREMIUM:    { label: "Premium",  cls: "bg-purple-50 dark:bg-purple-900/60 text-purple-700 dark:text-purple-300 border border-purple-800/40" },
  // Legacy alias'lar (eski orglar uchun ko'rsatish)
  BASIC:      { label: "Starter",  cls: "bg-neutral-200 dark:bg-neutral-700 text-neutral-300 dark:text-neutral-700 dark:text-neutral-300" },
  PRO:        { label: "Business", cls: "bg-blue-50 dark:bg-blue-900/60 text-blue-700 dark:text-blue-300 border border-blue-800/40" },
  ENTERPRISE: { label: "Premium",  cls: "bg-purple-50 dark:bg-purple-900/60 text-purple-700 dark:text-purple-300 border border-purple-800/40" },
};
const PLANS = ["STARTER", "BUSINESS", "PREMIUM"] as const;

type OrgForm = {
  name: string; subdomain: string; plan: "STARTER" | "BUSINESS" | "PREMIUM";
  adminName: string; adminPhone: string; adminEmail: string; adminPassword: string;
};
const EMPTY: OrgForm = { name:"", subdomain:"", plan:"STARTER", adminName:"", adminPhone:"", adminEmail:"", adminPassword:"" };

function Field({ label, required, children }: { label: string; required?: boolean; children: React.ReactNode }) {
  return (
    <div>
      <label className="block text-[11px] font-semibold text-neutral-500 dark:text-neutral-400 mb-1.5 uppercase tracking-wide">
...
const inputCls = "w-full h-9 px-3 text-[13px] rounded-lg border border-neutral-300 dark:border-neutral-700 bg-neutral-100 dark:bg-neutral-800 text-neutral-900 dark:text-white placeholder-neutral-400 dark:placeholder-neutral-600 outline-none focus:border-blue-500 transition-colors";
    ...
    <div className="max-w-7xl mx-auto space-y-5">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="w-full max-w-sm bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-2xl p-5 shadow-2xl">
            <div className="flex items-start justify-between mb-4">
              <div>
                <h3 className="text-[15px] font-bold text-neutral-900 dark:text-white">Admin parolini tiklash</h3>
            ...
            <div className="space-y-3">
              ...
              {resetErr && (
                <p className="text-[12px] text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-900/40 rounded-lg px-3 py-2">{resetErr}</p>
              )}
              {resetOk && (
      ...
      {showCreate && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4"
          ...
          <div className="bg-white dark:bg-neutral-900 border border-neutral-300 dark:border-neutral-700 rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            {/* Modal header */}
            <div className="flex items-center justify-between px-6 py-4 border-b border-neutral-200 dark:border-neutral-800 sticky top-0 bg-white dark:bg-neutral-900 z-10">
              <div>
                <h2 className="font-bold text-[15px] text-neutral-900 dark:text-white">Yangi tashkilot</h2>
              ...
              <button onClick={() => { setShowCreate(false); setForm(EMPTY); setCreateErr(""); }}
                className="w-7 h-7 flex items-center justify-center rounded-lg hover:bg-neutral-100 dark:hover:bg-neutral-800 text-neutral-500 dark:text-neutral-400 transition-colors">
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <Field label="Subdomain" required>
                    <div className="flex h-9 rounded-lg border border-neutral-300 dark:border-neutral-700 bg-neutral-100 dark:bg-neutral-800 overflow-hidden focus-within:border-blue-500 transition-colors">
                      <input
                        className="flex-1 px-3 text-[13px] bg-transparent text-neutral-900 dark:text-white placeholder-neutral-400 dark:placeholder-neutral-600 outline-none"
                        placeholder="yulduz"
                        value={form.subdomain}
                        onChange={e => setF("subdomain", e.target.value.toLowerCase().replace(/[^a-z0-9-]/g, ""))}
                      />
                      <span className="px-2 text-[10px] text-neutral-500 self-center whitespace-nowrap border-l border-neutral-300 dark:border-neutral-700">.oneroom.uz</span>
                    </div>
                  </Field>
                </div>
                    ...
                    {PLANS.map(p => (
                      <button key={p} onClick={() => setF("plan", p)}
                        className={cn(
                          ...
                          form.plan === p ? "bg-blue-600 border-blue-600 text-white" : "border-neutral-300 dark:border-neutral-700 text-neutral-500 dark:text-neutral-400 hover:border-neutral-400 dark:hover:border-neutral-600 hover:text-neutral-700 dark:hover:text-neutral-300"
                        )}>
                        {PLAN_CFG[p].label}
                      </button>
                    ))}
              ...
              {createErr && (
                <div className="flex items-center gap-2 bg-red-50 dark:bg-red-900/20 border border-red-800/40 rounded-xl px-3 py-2.5">
                  <AlertCircle className="w-3.5 h-3.5 text-red-600 dark:text-red-400 shrink-0" />
                  <p className="text-[12px] text-red-600 dark:text-red-400">{createErr}</p>
                </div>
              )}
            </div>
            ...
            <div className="px-6 pb-6 flex gap-2">
              ...
              <button onClick={() => { setShowCreate(false); setForm(EMPTY); setCreateErr(""); }}
                className="px-4 h-10 border border-neutral-300 dark:border-neutral-700 rounded-xl text-[13px] text-neutral-500 dark:text-neutral-400 hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-colors">
                Bekor
              </button>
            </div>
          </div>
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-xl font-black text-neutral-900 dark:text-white">Tashkilotlar</h1>
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <div className="relative flex-1 min-w-48">
          ...
          <input
            className="w-full h-9 pl-8 pr-3 text-[13px] rounded-xl border border-neutral-300 dark:border-neutral-700 bg-white dark:bg-neutral-900 text-neutral-900 dark:text-white placeholder-neutral-400 dark:placeholder-neutral-600 outline-none focus:border-neutral-400 dark:focus:border-neutral-600 transition-colors"
        ...
        <select value={planFilter} onChange={e => setPlanFilter(e.target.value)}
          className="h-9 px-3 text-[12px] rounded-xl border border-neutral-300 dark:border-neutral-700 bg-white dark:bg-neutral-900 text-neutral-300 dark:text-neutral-700 dark:text-neutral-300 outline-none focus:border-neutral-400 dark:focus:border-neutral-600 cursor-pointer">
        ...
        <div className="overflow-x-auto">
            ...
            <tbody>
              {isLoading
                ...
                : filtered.map(org => {
                    ...
                    return (
                      <tr key={org.id} className="border-b border-neutral-200 dark:border-neutral-800/50 hover:bg-neutral-100/20 dark:hover:bg-neutral-800/20 transition-colors">
                        {/* Name */}
                        <td className="px-4 py-3.5">
                          <div className="flex items-center gap-2.5">
                            <div className="w-8 h-8 rounded-xl bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-900/40 flex items-center justify-center shrink-0">
                              <Building2 className="w-3.5 h-3.5 text-blue-600 dark:text-blue-400" />
                            </div>
                            <div>
                              <p className="text-[13px] font-semibold text-neutral-900 dark:text-white">{org.name}</p>
                        ...
                        <td className="px-4 py-3.5">
                          <div className="flex items-center gap-1">
                            ...
                            <span className="text-[13px] font-bold text-green-600 dark:text-green-400">{org._count.students}</span>
                          </div>
                        </td>
                        <td className="px-4 py-3.5">
                          <div className="flex items-center gap-1">
                        ...
                        <td className="px-4 py-3.5">
                          <div className="flex items-center gap-1">
                            ...
                            <span className="text-[12px] font-bold text-emerald-600 dark:text-emerald-400">{fmtShort(org.revenue)}</span>
                          </div>
                        </td>
                        {/* Status */}
                        <td className="px-4 py-3.5">
                        ...
                        <td className="px-4 py-3.5">
                          <div className="flex items-center gap-1.5">
                            <button
                              ...
                              className="w-7 h-7 flex items-center justify-center rounded-lg bg-neutral-100 dark:bg-neutral-800 hover:bg-amber-50 dark:bg-amber-900/30 text-neutral-500 hover:text-amber-400 transition-colors">
                            ...
                            <button onClick={() => toggleActive(org.id, org.isActive)} disabled={togglingId === org.id}
                              className={cn(
                                ...
                                org.isActive
                                  ? "bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-400 hover:bg-red-50 dark:bg-red-900/50"
                                  : "bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 hover:bg-emerald-50 dark:bg-emerald-900/50"
                              )}>
                              {togglingId === org.id ? "..." : org.isActive ? "Bloklash" : "Ochish"}
                            </button>
                            <button onClick={() => deleteOrg(org.id, org.name)} disabled={deletingId === org.id}
                              className="w-7 h-7 flex items-center justify-center rounded-lg bg-neutral-100 dark:bg-neutral-800 hover:bg-red-50 dark:bg-red-900/30 text-neutral-500 hover:text-red-400 transition-colors disabled:opacity-40">
          ...
          {!isLoading && filtered.length === 0 && (
            <div className="py-16 text-center">
              <Building2 className="w-8 h-8 mx-auto mb-2 text-neutral-300 dark:text-neutral-700" />
              <p className="text-[13px] text-neutral-400 dark:text-neutral-600 mb-3">
