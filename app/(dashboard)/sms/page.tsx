...
export default function SmsPage() {
 ...
 className="flex-1 h-9 bg-indigo-600 hover:bg-indigo-700 dark:bg-indigo-500 text-white text-[13px]">
        ...
        <div className="space-y-3">
            ...
            {(sms?.packages ?? []).map(p => (
              <button key={p.quantity} type="button" onClick={() => { setBuyQty(p.quantity); setBuyCustom(""); }}
                className={cn("rounded-xl border-2 p-3 text-center transition-all",
                  !buyCustom && buyQty === p.quantity ? "border-neutral-900 dark:border-neutral-100 glass-soft"
          ...
          <div className="glass-soft rounded-xl p-3 flex justify-between text-[13px]">
