...
export function RoomGrid({
  groups, rooms, blockColor,
}: {
  groups: Guruh[];
  rooms: { id: string; name: string }[];
  /** Sahifadagi rang mantig'i — palitra ikki joyda takrorlanmasin. */
  blockColor: (g: Guruh, index: number) => string;
}) {
  ...
  /**
   * USTUNLAR — faqat SHU tabda band bo'lgan xonalar, oxirida
   * "Xonasiz" ustuni (xonasi yo'q guruh yo'qolib qolmasin).
   */
  const ustunlar = useMemo(() => {
    const band = new Set(korinadi.map((g) => g.room?.id).filter(Boolean) as string[]);
    const list = rooms.filter((r) => band.has(r.id));
    const xonasizBor = korinadi.some((g) => !g.room?.id);
  ...
  const vaqtlar = useMemo(
    () => [...new Set(korinadi.map((g) => g.startTime))].sort(),
    [korinadi]);

  const katak = (vaqt: string, xonaId: string) =>
    korinadi.filter((g) => g.startTime === vaqt && (g.room?.id ?? "__yoq__") === xonaId);
        ...
        <div className="overflow-x-auto">
          <div className="min-w-max">
            <div className="flex border-b border-white/50 dark:border-white/10">
              ...
              {ustunlar.map((r) => (
                <div key={r.id}
                  className="w-[190px] shrink-0 px-3 py-2.5 text-[12px] font-bold
                    text-neutral-600 dark:text-neutral-300 border-l border-white/50 dark:border-white/10">
                  {r.name}
                </div>
              ))}
            </div>
            ...
            {vaqtlar.map((vaqt) => (
              <div key={vaqt} className="flex border-b border-white/50 dark:border-white/10 last:border-0">
                <div className="w-16 shrink-0 px-2 py-3 text-[11px] font-bold tabular-nums
                  text-neutral-500 dark:text-neutral-400">
                  {vaqt}
                </div>
                {ustunlar.map((r) => (
                  <div key={r.id}
                    className="w-[190px] shrink-0 p-1.5 space-y-1.5
                      border-l border-white/50 dark:border-white/10">
                    {katak(vaqt, r.id).map((g, i) => (
                      <Link key={g.id} href={`/groups/${g.id}`}
                        className={cn("block rounded-lg border-l-4 px-2 py-1.5 transition-opacity hover:opacity-80",
                          blockColor(g, i))}>
                        {g.course?.name && (
